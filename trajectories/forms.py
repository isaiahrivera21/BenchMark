...
class TrajectoryForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if goal_type == Trajectory.FOOD:
            ...
            if focus_area not in valid_choices:
                self.add_error(
                    "focus_area",
                    f'Invalid focus area for food goal. Must be one of: {", ".join(valid_choices)}',
                )

        if objective == Trajectory.INCREASE and future_amount <= current_amount:
            self.add_error(
                "future_amount",
                'For "INCREASE" objective, future_amount must be greater than current_amount.',
            )
        elif objective == Trajectory.DECREASE and future_amount >= current_amount:
            self.add_error(
                "future_amount",
                'For "DECREASE" objective, future_amount must be less than current_amount.',
            )
        elif objective == Trajectory.SAME:
            if same_value is None:
                self.add_error(
                    "same_value", 'This field is required for "SAME" objective.'
                )
            else:
    ...
    def save(self, commit=True):
        ...
        if instance.objective == Trajectory.SAME:
            instance.current_amount = instance.future_amount = self.cleaned_data.get(
                "same_value"
            )
        if commit:
            instance.save()
